	...
	public static void main(String[] args) throws InterruptedException {
		...
		try {
			...
			if(flag){
				...
				//生成题目文件Questions.txt
				BufferedWriter bw1=new BufferedWriter(new FileWriter("src/Questions.txt"));
				...
				bw1.close();
				//生成答案文件Answers.txt
				BufferedWriter bw2=new BufferedWriter(new FileWriter("src/Answers.txt"));
				for(int i=0;i<questionBase.size();i++){
				...
				bw2.close();
				//生成答题文件Exercises.txt
				BufferedWriter bw3=new BufferedWriter(new FileWriter("src/Exercises.txt"));
				for(int i=0;i<questionBase.size();i++){
					String s=i+1+".\r\n";
					bw3.write(s);
				}
			...
			//生成分数文件Grade.txt
			BufferedWriter bw=new BufferedWriter(new FileWriter("src/Grade.txt"));
